...
class App(Cmd):
	...
	spotify = {'USERNAME': '', 'CLIENT_ID': '', 'CLIENT_SECRET': '', 'REDIRECT_URI': ''}		#dictionary for Spotify attributes
	spotify_api = None																			#attribute for Spotify API object

	lyrics_genius = {'TOKEN': ''}																#dictionary for Lyrics Genius attributes
	lyrics_genius_api = None																	#attribute for Lyrics Genius API object
	...
	VAR:
		...
		if arg[0] == 'SPOTIFY':														#if first argument is 'SPOTIFY':
			if arg[1] in list(self.spotify.keys()):									#if second argument is in spotify keys
				...
				return 																#else log error and return
			...
			return 																	#if first argumernt is invalid, log error and return
